...
def secret_value(name: str) -> str:
    """Read a secret from a CSI-mounted file, with env fallback for local tests."""
    ...
    if value:
        return value
    raise RuntimeError(f"Missing required secret: set {name}_FILE or {name}")
...
class Settings:
    ...
    def from_env(cls) -> "Settings":
        required = ("POSTGRES_HOST", "POSTGRES_DB", "MINIO_ENDPOINT", "MINIO_BUCKET")
        ...
        return cls(
            ...
            minio_access_key=secret_value("MINIO_ACCESS_KEY"),
            minio_secret_key=secret_value("MINIO_SECRET_KEY"),
...
class PostgresRepository:
    ...
    def initialize(self) -> None:
        with self._connect() as connection:
            # Schema ownership stays outside short-lived application identities.
            connection.execute("SELECT 1 FROM stored_objects LIMIT 0")
